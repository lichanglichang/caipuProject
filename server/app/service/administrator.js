...
class UserService extends Service {
    ...
    //管理获取所有用户信息
    async getAllUser(num, kw) {
        if (kw) {
            let userResult1 = await this.app.mysql.query(` SELECT * FROM user where nickname  like '%${kw}%'`)
            return userResult1;
        } else {
            let local = (num - 1) * 5;
            let userResult1 = await this.app.mysql.query(` select * from user limit ${local},5 `)
            return userResult1;
        }
    }
    ...
    async addUser(username, password, nickname, sex) {
        ...
        if (username && password && nickname && sex) {
            ...
            if (userResult.length == 0) {
                let addResult = await this.app.mysql.query
    ...
    async getAllGoods(num, kw) {
        if (kw) {
            let userResult1 = await this.app.mysql.query(` SELECT * FROM goods where address  like '%${kw}%'`)
            return userResult1;
        } else {
            let local = (num - 1) * 5;
            let userResult1 = await this.app.mysql.query(` select * from goods limit ${local},5 `)
            return userResult1;
        }
    }
    ...
    async getAllRecipe(num, kw) {
        if (kw) {
            let userResult1 = await this.app.mysql.query(` SELECT * FROM recipe where nickname  like '%${kw}%'`)
            return userResult1;
        } else {
            let local = (num - 1) * 5;
            let userResult1 = await this.app.mysql.query(` select * from recipe limit ${local},8 `)
            return userResult1;
        }
    }
    //删除菜谱
    ...
    async getAllMenu(num, kw) {
        if (kw) {
            let userResult1 = await this.app.mysql.query(` SELECT * FROM menu where menuname  like '%${kw}%'`)
            return userResult1;
        } else {
            let local = (num - 1) * 5;
            let userResult1 = await this.app.mysql.query(` select * from menu limit ${local},8 `)
            return userResult1;
        }
    }
    //删除菜单
    ...
    async getAllNotes(num, kw) {
        if (kw) {
            let userResult1 = await this.app.mysql.query(` SELECT * FROM notes where title  like '%${kw}%'`)
            return userResult1;
        } else {
            let local = (num - 1) * 5;
            let userResult1 = await this.app.mysql.query(` select * from notes limit ${local},8 `)
            return userResult1;
        }
    }
    //删除笔记
