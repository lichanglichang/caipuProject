...
class UserService extends Service {
    ...
    async showrepicedes(repiceid) {
		let repiceids = await this.app.mysql.query(`select * from recipe where id='${repiceid}' `);
    ...
    async showmenusdes(menusid){
        ...
        let result1=await this.app.mysql.query(`select * from menu where menuid='${menusid}'`);
    ...
    async showmenuone(newarr){
        ...
        if(narr){
            for(let i=0;i<narr.length;i++){
                let result=await this.app.mysql.query(`select * from recipe where id='${narr[i]}'`); 
    ...
    async login(username, password) {
        let message = await this.app.mysql.
...
async getuser(username) {
    ...
    let userinfor = await this.app.mysql.query(`SELECT * FROM user WHERE username = "${username}"`);
    return userinfor
...
async getguanzu(username) {
    ...
    let guanzuid = await this.app.mysql.query(`SELECT guanzu FROM user WHERE username = "${username}"`);
   let arr = [];
for(let i=0;i<guanzuid[0].guanzu.length;i++){
     let user = await this.app.mysql.query(`SELECT * FROM user WHERE id = "${guanzuid[0].guanzu[i]}"`); 
     arr=arr.concat(user);
}
    return arr;
