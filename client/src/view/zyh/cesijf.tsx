...
export default function Cesijf() {
        ...
        if (list.length !== 0) {
            return (
                <>
                    ...
                    <div onClick={sfzq}>
                        <img src={JSON.parse(list[num].optionA).url} alt={JSON.parse(list[num].optionA).name} />
                        <img src={JSON.parse(list[num].optionB).url} alt={JSON.parse(list[num].optionB).name} />
                        <img src={JSON.parse(list[num].optionC).url} alt={JSON.parse(list[num].optionC).name} />
                        <img src={JSON.parse(list[num].optionD).url} alt={JSON.parse(list[num].optionD).name} />
                    </div>
                    <div>
                    ...
                    <div style={{ alignSelf: 'flex-end', color: '#999999' }}>
                        <span style={{ color: '#7CC331' }}>{localStorage.getItem("tishu")}</span> /10
                    </div>
                </>
            )
        }
    ...
    return (
        <div ref={qb} style={{ width: '100vw', minHeight:'100vh', backgroundColor:'white'}}>
            <div className={cesijf.logo}>
            <Link to={{pathname:'/'}}><img src={jflogo} alt="" /></Link>
            </div>
            <div className={cesijf.bg}>
                <div className={cesijf.tm}>
                    {jiemian()}
                </div>
                <div style={{ alignSelf: 'flex-center', marginBottom: '40px' }}>
                    <button onClick={xyt} ref={btn} className={cesijf.anniu}>下一题</button>
                </div>
                <div ref={cjandzy} className={cesijf.xx}>
                    ...
                    <button className={cesijf.fhsy} onClick={cxcs}>重新测试</button>
                    <Link to={{ pathname: '/' }}><button className={cesijf.fhsy}>返回首页</button></Link>
