...
export default function Comicdetails() {
    ...
    let obj = useParams();
    function details(){
       return imgbox.map((item:any)=>{
           if(obj.cid==item.id){
               return (
                   <div key={item.id} style={{marginLeft:'50px'}}>
                        <div style={{fontSize:'20px',fontWeight:'bold'}}>{item.des}</div>
    ...
    return (
        <>
            <Header></Header>
            <div style={{width:'1050px',margin:'30px auto',display:'flex'}}>
                {details()}
